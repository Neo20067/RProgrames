...
barplot(termF, las=2, ylab="number of Words", col="darkgreen", main="Bar plot for Nokia words")
...
plot(fit,main="Cluster dendrogram for Nokia words")
rect.hclust(fit,k=6)
...
d1=daisy(kdoc)
d2=d1^2
clusplot(kdoc,kmeansResult$cl, color=TRUE, shade=TRUE, labels=4, lines=0, main="k-means clusplot for Nokia tweets")
sk=silhouette(kmeansResult$cl, d2)
plot(sk,main="k-mean silhouette plot for Nokia tweets")
...
clusplot(pamResult,pamResult$cl, color=TRUE, labels=4, lines=0, main="k-medoids clusplot for Nokia tweets")
plot(pamResult, color=F, labels=4, lines=0, cex=.8, col.clus=1,col.p=pamResult$clustering,main="k-medoids silhouette plots for Nokia Tweets")
