setwd("j:/programs/clustering")
library(rjson)
library(RJSONIO)
library(tm)
iTweets=fromJSON("samsung_Processed_tweets.json")
...
barplot(termF, las=2, ylab="number of Words",col="orange", main="Bar plot for Samsung words")

#which words are associated with iphone
findAssocs(itdm, c("samsung","galaxy","apple","iphone"), 0.25)
...
plot(fit,main="Cluster dendrogram for Samsung words")
rect.hclust(fit,k=7,main="Cluster dendrogram for Samsung words")
...
d1=daisy(kdoc)
d2=d1^2
clusplot(kdoc,kmeansResult$cl, color=TRUE, labels=4, lines=0, main="k-means clusplot for Samsung tweets")
sk=silhouette(kmeansResult$cl, d2)
plot(sk,main="k-mean silhouette plot for Samsung tweets")
...
layout(matrix(c(1,3),,1))#set to two graphs per page
clusplot(pamResult,pamResult$cl, color=TRUE, labels=4, lines=0, main="k-medoids clusplot for Samsung tweets")
plot(pamResult, color=F, labels=4, lines=0, cex=.8, col.clus=1,col.p=pamResult$clustering,main="k-medoids silhouette plots for Samsung Tweets")
